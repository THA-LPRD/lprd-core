...
function formatTimeValue(value: TimeValue, showSeconds: boolean): string {
    ...
    const minuteStr =
        value.minute !== undefined ? value.minute.toString().padStart(2, '0') : DEFAULT_SEGMENT_PLACEHOLDER;
    const secondStr =
        value.second !== undefined ? value.second.toString().padStart(2, '0') : DEFAULT_SEGMENT_PLACEHOLDER;
...
interface TimePickerLabelProps extends React.ComponentProps<'label'>, useRender.ComponentProps<'label'> {}
...
function TimePickerInputGroup(props: DivProps) {
    ...
    const onClick = React.useCallback(
        ...
        [onClickProp, disabled, readOnly, inputGroupClickAction, store, triggerRef, inputGroupRef, openOnFocus],
    );
...
function TimePickerInput(props: TimePickerInputProps) {
    ...
    const onClick = React.useCallback(
        (event: React.MouseEvent<InputElement>) => {
            ...
            if (openOnFocus && !isDisabled && !isReadOnly && !store.getState().open) {
                store.setState('openedViaFocus', true);
                store.setState('open', true);
            }
        ...
        [onClickProp, openOnFocus, isDisabled, isReadOnly, store],
    );
...
function TimePickerContent(props: TimePickerContentProps) {
    ...
    const onInteractOutside: NonNullable<PopoverContentProps['onInteractOutside']> = React.useCallback(
        ...
        [onInteractOutsideProp, inputGroupRef],
    );
    ...
    return (
        <TimePickerGroupContext.Provider value={groupContextValue}>
            <PopoverContent
                ...
                {...contentProps}
                className={cn('flex w-auto max-w-(--anchor-width) p-0', className)}
...
function TimePickerColumnItem(props: TimePickerColumnItemProps) {
    ...
    return (
        <button
            ...
            className={cn(
                'w-full rounded px-3 py-1.5 text-left text-sm outline-none transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50',
                selected &&
...
function TimePickerHour(props: TimePickerHourProps) {
    ...
    const selectedDisplayHour =
        timeValue?.hour === undefined ? null : is12Hour ? to12Hour(timeValue.hour).hour : timeValue.hour;

    const hoursContent = hours.map((hour) => (
        <TimePickerColumnItem
            key={hour}
            value={hour}
            selected={selectedDisplayHour === hour}
            format={format}
...
function TimePickerPeriod(props: DivProps) {
    ...
    const currentPeriod = timeValue?.hour === undefined ? null : to12Hour(timeValue.hour).period;
...
interface TimePickerSeparatorProps extends React.ComponentProps<'span'>, useRender.ComponentProps<'span'> {}
