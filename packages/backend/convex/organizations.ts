...
async function getOrganizationByExternalId(ctx: Parameters<typeof resolveAuthorization>[0], externalId: string) {
    return ctx.db
        .query('organizations')
...
export const upsertFromWebhook = internalMutation({
    ...
    handler: async (ctx, args) => {
        const existing = await getOrganizationByExternalId(ctx, args.externalId);
        const now = Date.now();
