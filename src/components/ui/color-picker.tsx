...
interface ColorPickerProps extends Omit<React.ComponentProps<'div'>, 'onValueChange'> {
    value?: string;
    defaultValue?: string;
...
function ColorPickerImpl(props: ColorPickerImplProps) {
    ...
    const contextValue = React.useMemo<ColorPickerContextValue>(
        ...
        [disabled, inline, readOnly, required],
    );
...
function ColorPickerTrigger(props: React.ComponentProps<'button'>) {
    ...
    return <PopoverTrigger data-slot="color-picker-trigger" disabled={isDisabled} {...triggerProps} />;
}

// --- ColorPickerContent ---

function ColorPickerContent(props: React.ComponentProps<typeof PopoverContent> & { className?: string }) {
    ...
    if (context.inline) {
        return (
            <div data-slot="color-picker-content" className={cn('flex w-[340px] flex-col gap-4 p-4', className)}>
                {children}
            </div>
        );
    }
...
function ColorPickerHueSlider(props: Omit<SliderPrimitive.Root.Props, 'onValueChange'>) {
...
function ColorPickerAlphaSlider(props: Omit<SliderPrimitive.Root.Props, 'onValueChange'>) {
    const { className, ...sliderProps } = props;
