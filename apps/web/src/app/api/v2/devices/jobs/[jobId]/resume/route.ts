...
import type { WorkerJobPayload } from '@shared/jobs/types';
...
export async function POST(request: Request, context: { params: Promise<{ jobId: string }> }) {
    try {
        ...
        const queuedJob = await fetchMutation(api.jobs.deviceJobs.resume, { id: job._id }, { token });
        ...
        console.error('Device job resume error:', error);
