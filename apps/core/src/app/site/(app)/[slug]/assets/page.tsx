...
const ASSET_SKELETON_IDS = Array.from({ length: 10 }, (_, index) => `asset-skeleton-${index}`);
        ...
        <div className="flex flex-col gap-6 p-6">
            ...
            {assets === undefined ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    {ASSET_SKELETON_IDS.map((id) => (
                        <div key={id} className="aspect-square rounded-lg bg-muted animate-pulse" />
                    ))}
                </div>
            ) : assets.length === 0 ? (
