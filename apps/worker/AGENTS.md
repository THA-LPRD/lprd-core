...
- **Plugin-triggered batch renders**: When a plugin pushes new data it may trigger renders across many devices simultaneously. The web app accepts the incoming data, logs the jobs, and returns `200` immediately. Worker drains the queue at its own pace without making the plugin wait.
...
- Modify how job results and status are reported back to the web app
...
- Requires web app base URL and M2M credentials (`CORE_BASE_URL`, `WORKER_CLIENT_ID`, `WORKER_CLIENT_SECRET`, `WORKOS_AUTHKIT_DOMAIN`).
