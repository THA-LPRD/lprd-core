...
function formatJobError(error: unknown): string {
    ...
    return `${message.slice(0, MAX_JOB_ERROR_MESSAGE_LENGTH)}... [truncated]`;
}

async function reportJobFailure(jobStatusPath: string, error: unknown) {
    try {
        ...
        console.error(
            `[worker] Failed to mark job as failed at ${jobStatusPath}:`,
    ...
    const worker = new Worker<WorkerJobPayload>(
        ...
        async (job) => {
            ...
            try {
                ...
                if (jobStatusPath) {
                    await reportJobFailure(jobStatusPath, error);
                }
                throw error;
            }
        },
