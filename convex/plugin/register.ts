...
export const handlePluginRegister = httpAction(async (ctx, request) => {
	try {
		...
		if (!name || !version || !base_url) {
			return new Response(
				JSON.stringify({error: 'name, version, and base_url are required'}),
				{status: 400, headers: {'Content-Type': 'application/json'}},
			);
		}

		const id = uuidv4();
		const result = await ctx.runMutation(internal.plugins.registerPlugin, {
			id,
			name,
			version,
		...
		return new Response(
			JSON.stringify({registration_id: result.id}),
